...
;
; O banco fica num servidor PostgreSQL, não neste PC: desinstalar ou
; atualizar o programa nunca toca nele. Aqui fica só o que é deste PC — a
; conexão com o servidor, as preferências e os backups feitos por ele — em
; %PROGRAMDATA%\ControleDeLucros\, fora da pasta de instalação, então
; também sobrevive a desinstalar e reinstalar.
;
; A conexão NÃO é perguntada aqui: o instalador roda como administrador, e o
; administrador não enxerga a rede do jeito de quem usa o PC. O programa
; pergunta na primeira abertura, já como o usuário.
;
; %PROGRAMDATA% e não %LOCALAPPDATA%: a conexão configurada vale pra todos
; os usuários do Windows daquele PC. Ver [Dirs] no fim deste arquivo — é lá
; que a pasta ganha permissão de escrita pra usuário comum.
...
; mesmo criou: quem configurou a conexão seria o único a conseguir
; corrigi-la depois, e as preferências de um travariam as do outro.
...
[Run]
; runasoriginaluser: o instalador roda como administrador, e a primeira
; abertura é a que configura a conexão — tem que ser como quem usa o PC.
